...
import { CartItem, Transaction } from "@/models/Transactions";
...
export default function DashboardPage() {
  ...
  const getTodayMetrics = useTransactionStore((state) => state.getTodayMetrics);

  const { revenue, profit, count } = getTodayMetrics();
  ...
  transactions.forEach((t: Transaction) => {
    t.items.forEach((item: CartItem) => {
      ...
      if (!itemSalesCount[id]) {
        itemSalesCount[id] = { name: item.name, qty: 0, revenue: 0 };
      }
      itemSalesCount[id].qty += item.quantity;
      itemSalesCount[id].revenue += item.quantity * item.price;
    });
  });
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold tracking-tight">₹{revenue.toLocaleString("en-IN")}</span>
              </div>
            </CardContent>
          </Card>
              ...
              {transactions.length === 0 ? (
                ...
                <div className="space-y-6">
                  {transactions.slice(0, 50).map((txn) => (
                    <div key={txn._id} className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold tabular-nums">₹{txn.total_amount}</p>
                        <p className="text-[10px] text-emerald-500 font-medium">Profit ₹{Math.floor(txn.total_profit)}</p>
                      </div>
                    </div>
                  ))}
                </div>
